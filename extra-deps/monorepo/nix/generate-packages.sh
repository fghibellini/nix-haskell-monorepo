...
APACKAGE_FILE_PATTERN="$($WITH_HPACK && echo 'package.yaml' || echo '*.cabal')"
for row in $(grep -R --include "$APACKAGE_FILE_PATTERN" --exclude-dir '.stack-work' '^name:' "$CODE_ROOT" | sed 's/:name:\s*/:/'); do
